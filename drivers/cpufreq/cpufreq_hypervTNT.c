/*
 *  drivers/cpufreq/cpufreq_HYPERvTNT.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_HYPERvTNT
static
#endif
struct cpufreq_governor cpufreq_gov_HYPERvTNT = {
       .name                   = "HYPERvTNT",
...
#ifdef SUSPEND_FREQ_ON
// HYPERvTNT suspend mods (Thanks to Imoseyon)
static unsigned int suspended = 0;
static void HYPERvTNT_suspend(int suspend)
{
        ...
        if (!suspend) { // resume at max speed:
			...
			CPUFREQ_RELATION_L);
                pr_info("[HYPERvTNT] HYPERvTNT awake at %d\n", dbs_info->cur_policy->cur);
        } else {
                suspended = 1;
		// let's give it a little breathing room
                ...
                pr_info("[HYPERvTNT] HYPERvTNT suspended at %d\n", dbs_info->cur_policy->cur);
        }
}

static void HYPERvTNT__power_early_suspend(struct early_suspend *handler) {
       HYPERvTNT_suspend(1);
}

static void hypervTNT__power_late_resume(struct early_suspend *handler) {
       HYPERvTNT_suspend(0);
}

static struct early_suspend HYPERvTNT_suspend = {
        .suspend = HYPERvTNT_early_suspend,
        .resume = HYPERvTNT_late_resume,
...
static void hypervTNT__power_powersave_bias_init_cpu(int cpu)
{
...
static void HYPERvTNT_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		hypervTNT__power_powersave_bias_init_cpu(i);
	}
}
...
/* cpufreq_hypervTNT__power Governor Tunables */
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		/*
		 * For the purpose of HYPERvTNT, waiting for disk IO is an
...
unsigned int prev_up_threshold_HYPERvTNT;
unsigned int prev_freq_step_HYPERvTNT;
unsigned int prev_sampling_rate_HYPERvTNT;
static void cpufreq_HYPERvTNT_early_suspend(struct early_suspend *h)
{
	dbs_tuners_ins.early_suspend = 1;
	prev_freq_step_HYPERvTNT = dbs_tuners_ins.freq_step;
	prev_sampling_rate_HYPERvTNT = dbs_tuners_ins.sampling_rate;
	prev_up_threshold_HYPERvTNT = dbs_tuners_ins.up_threshold;
...
static void cpufreq_HYPERvTNT_late_resume(struct early_suspend *h)
{
	dbs_tuners_ins.early_suspend = -1;
	dbs_tuners_ins.freq_step = prev_freq_step_HYPERvTNT;
	dbs_tuners_ins.sampling_rate = prev_sampling_rate_HYPERvTNT;
	dbs_tuners_ins.up_threshold = prev_up_threshold_HYPERvTNT;
}
#endif
#endif
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		hypervTNT__power_powersave_bias_init_cpu(cpu);
		/*
		...
		register_early_suspend(&HYPERvTNT_power_suspend);
#endif
#endif
		break;
	...
	case CPUFREQ_GOV_STOP:
		...
		unregister_early_suspend(&HYPERvTNT_power_suspend);
#endif
#endif
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	early_suspend.suspend = cpufreq_HYPERvTNT_early_suspend;
	early_suspend.resume = cpufreq_HYPERvTNT_late_resume;
#endif
#endif

	return cpufreq_register_governor(&cpufreq_gov_HYPERvTNT);
}

static void __exit cpufreq_gov_dbs_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_HYPERvTNT);
}

MODULE_AUTHOR("Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>");
MODULE_AUTHOR("Alexey Starikovskiy <alexey.y.starikovskiy@intel.com>");
MODULE_AUTHOR("Dorimanx <yuri@bynet.co.il>");
MODULE_DESCRIPTION("'cpufreq_HYPERvTNT' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors" 
"Module include, IOWAIT,FREQ-TUNERS,EARLY-SUSPEND,SUSPEND-FREQ");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_HYPERvTNT
